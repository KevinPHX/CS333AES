...
if __name__ == '__main__':
    ...
    # data.to_csv("../outputs/identification_bert.csv")
    # # save models
    # with open('../models/identification_probability_bert.json', 'w') as f:
    #     json.dump(identifier.probability, f)
    
    # data = pd.read_csv("../outputs/identification_bert.csv")
    ...
    # with open("../models/probability_bert.json", "w") as f:
    #     json.dump(argclass.probability, f)
    # with open("../models/dependency_bert.json", "w") as f:
    #     json.dump(argclass.dependency_tuples, f)
    # with open('../models/classification_vectorizer_bert.pkl', 'wb') as f:
    ...
    #     with open(f"../outputs/classification_bert/{key}.json", "w") as f:
    #         json.dump(essays[key], f)
    
    print("Relation Identification")

    for essay_name in train_text: 
        essay_name = essay_name.split('/')[-1]
        ...
        with open(f"../outputs/relations_bert/{essay_name}.json", "w") as file:
    ...
    for essay_file in train_text: 
        ...
        stance.process_data()
        with open(f"../outputs/stance_bert/{essay_name}.json", "w") as file:
